    ...
    for defect in valve.defect_list:
        ...
        for defect_type in defect_names:
            for defect_name_variant in defect_names[defect_type]:
                if (
                    defect.defect_type is None
                    and defect.defect_description.upper().find(
                        defect_name_variant.upper()
                    )
                    != -1
                ):
        ...
        for defect_class in defect_classes:
            for defect_type in defect_classes[defect_class]:
                if (
                    ...
                    and defect.defect_class is None
                    and defect.defect_type.upper().find(defect_type.upper()) != -1
                ):
