    ...
    for main_type_name in whitelist:
        for type_name in whitelist[main_type_name]:
            for type_name_variant in whitelist[main_type_name][type_name]:
                if (
                    valve.valve_name.upper().find(type_name_variant.upper()) != -1
                    or valve.element_name.upper().find(type_name_variant.upper()) != -1
                    or valve.get_descritpion().upper().find(type_name_variant.upper())
                    != -1
                ):
                    result = True
                    valve.valve_type = type_name_variant
                    valve.main_valve_type = main_type_name
                    return result
        if (
            valve.valve_name.upper().find(main_type_name.upper()) != -1
            or valve.element_name.upper().find(main_type_name.upper()) != -1
            or valve.get_descritpion().upper().find(main_type_name.upper()) != -1
        ):
            result = True
            valve.main_valve_type = main_type_name
    return result
